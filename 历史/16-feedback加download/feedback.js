...
document.getElementById('downloadRubric')?.addEventListener('click', () => {
  if (!currentRubricInfo) {
    alert('Rubric file not found. Please ensure the rubric has been uploaded.');
    return;
  }
  
  // 创建下载链接
  const link = document.createElement('a');
  link.href = currentRubricInfo.download_url;
  link.download = currentRubricInfo.file_name;
  link.click();
});
