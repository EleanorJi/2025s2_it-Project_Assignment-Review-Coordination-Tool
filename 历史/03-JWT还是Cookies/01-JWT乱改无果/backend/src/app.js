...
// ==================== 新增：路由拦截中间件 ====================
// 在静态文件服务之前添加，确保先执行权限检查

// 保护 Coordinator 页面
app.get('/Coordinator/coordinator-dashboard.html', authenticate, (req, res, next) => {
...
// 保护 Marker 页面
app.get('/Marker/marker-dashboard.html', authenticate, (req, res, next) => {
...
// 保护其他需要权限的页面（根据需要添加）
app.get('/Coordinator/invite.html', authenticate, (req, res, next) => {
  if (req.user && req.user.role === 'COORDINATOR') {
    next();
  } else {
    res.status(403).send('Forbidden: Insufficient permissions');
...
app.get('/Coordinator/upload-assignment.html', authenticate, (req, res, next) => {
  if (req.user && req.user.role === 'COORDINATOR') {
    next();
  } else {
    res.status(403).send('Forbidden: Insufficient permissions');
