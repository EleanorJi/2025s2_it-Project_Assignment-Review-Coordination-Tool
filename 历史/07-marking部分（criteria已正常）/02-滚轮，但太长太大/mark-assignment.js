...
(() => {
  ...
  async function loadAndDisplayPdf() {
    try {
      ...
      if (!filesResponse.ok) throw new Error('Failed to get assignment files');
  ...
  function initPagesContainer() {
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pageDiv.innerHTML = `
        <div class="page-placeholder" style="width: 800px; height: 1000px; background: #f5f5f5; display: flex; align-items: center; justify-content: center; color: #666;">
          Loading page ${i}...
        </div>
      `;
  ...
  function setupScrollHandling() {
    ...
    container.addEventListener('wheel', (e) => {
      e.preventDefault();
      container.scrollTop += e.deltaY * 0.5; // 降低滚动速度，更平滑
    }, { passive: false });
  }
  ...
  function renderVisiblePages() {
    ...
    // 计算可见页面范围
    const visiblePages = getVisiblePages(viewportTop, viewportBottom);

    // 渲染可见页面
    renderPages(visiblePages);
  }

  function getVisiblePages(viewportTop, viewportBottom) {
    const pages = [];
    ...
    pageElements.forEach((pageEl, index) => {
      ...
      // 如果页面在视口内或附近（预加载前后2页）
      if (pageBottom >= viewportTop - 1000 && pageTop <= viewportBottom + 1000) {
        pages.push(index + 1);
      }
    });
  ...
  async function renderPage(pageNum) {
    ...
    const viewport = page.getViewport({ scale: 1.5 }); // 固定缩放比例，适合阅读
  ...
  function updateCurrentPage() {
    ...
    for (let i = 0; i < pageElements.length; i++) {
      const pageTop = pageElements[i].offsetTop;
      const pageBottom = pageTop + pageElements[i].offsetHeight;

      if (viewportTop >= pageTop && viewportTop <= pageBottom) {
        currentPage = i + 1;
        break;
      }
    }

    if (currentPage !== currentPdfPage) {
  ...
  function setupDocumentNavigation() {
    ...
    viewBtn?.addEventListener('click', () => {
      alert('Text view not available for PDF documents');
    });
