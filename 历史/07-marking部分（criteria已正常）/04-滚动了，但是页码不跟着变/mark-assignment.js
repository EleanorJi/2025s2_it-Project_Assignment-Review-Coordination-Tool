...
(() => {
  ...
  async function renderAllPages() {
      try {
          ...
          for (let i = 1; i <= totalPdfPages; i++) {
              ...
              const pageHeight = canvas.offsetHeight; // 或者使用 viewport.height 加上一个固定的间距
  ...
  function updateCurrentPageFromScroll() {
      const pdfViewer = document.getElementById('pdf-viewer');
      const scrollTop = pdfViewer.scrollTop; // 当前滚动位置
      const viewerHeight = pdfViewer.clientHeight; // 容器可见区域高度

      let accumulatedHeight = 0;
      let newCurrentPage = 1;

      // 遍历所有页面，找到当前滚动位置所在的页面
      for (let i = 0; i < pageHeights.length; i++) {
          // 计算到第i页底部的累计高度
          accumulatedHeight += pageHeights[i];

          // 更稳健的判断逻辑：如果当前滚动位置（加上可见区域中部的一个偏移量）小于累计高度，则认为当前处于第i+1页
          // 使用可见区域中部作为参考点，比顶部或底部更稳定:cite[3]
      ...
      // 防止页面编号超出范围
      if (newCurrentPage > totalPdfPages) {
          newCurrentPage = totalPdfPages;
      }
