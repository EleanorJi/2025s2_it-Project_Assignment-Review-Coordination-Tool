...
function genToken() {
  return crypto.randomBytes(32).toString('hex'); // 发给对方
}
function daysFromNow(n) {
  return new Date(Date.now() + n*24*60*60*1000);
}

// 简单鉴权：用请求头 x-user-id 找当前用户（演示）
async function requireAuth(req, res, next) {
  const id = req.header('x-user-id');
  if (!id) return res.status(401).json({ success:false, message:'No user' });
  const u = await db.query('select id, role from users where id=$1', [id]);
  if (!u.rowCount) return res.status(401).json({ success:false, message:'Invalid user' });
  req.user = u.rows[0];
  next();
}
function requireCoordinator(req, res, next) {
  if (req.user.role !== 'coordinator') return res.status(403).json({ success:false, message:'Forbidden' });
  next();
}
const app = express();
const port = 3000;
